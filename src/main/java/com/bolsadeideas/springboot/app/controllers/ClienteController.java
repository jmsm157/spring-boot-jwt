...
//		Cliente cliente = clienteService.findOne(id);
		Cliente cliente = clienteService.fetchByIdWhithFacturas(id);
		if (cliente == null) {
	...
	public String listar(@RequestParam(name = "page", defaultValue = "0") int page, Model model,
		...
		if (auth != null) {
			logger.info(
					"Utilizando de forma estática SecurityContextHolder.getContext().getAuthentication(): Usuario autenticado, tu username es: "
							.concat(auth.getName()));
		}
...
//		}
	...
	public String editar(@PathVariable(value = "id") Long id, Map<String, Object> model, RedirectAttributes flash) {
		...
		if (id > 0) {
			...
			if (cliente == null) {
				flash.addFlashAttribute("error", "El ID del cliente no no exist en BBDD!");
				return "redirect:/listar";
			}
		} else {
			flash.addFlashAttribute("error", "El ID del cliente no puede cero!");
			return "redirect:/listar";
		}
		model.put("cliente", cliente);
	...
	public String guardar(@Valid Cliente cliente, BindingResult result, @RequestParam("file") MultipartFile foto,
		...
		String mesajeFlash = (cliente.getId() != null) ? "Cliente editado con éxito!" : "Cliente creado con éxito!";
